...
def keyFrameExtractor_diff(video_path_id: tuple[str, int], output_path: str,
                           frame_step=6, max_frame=12, method='to_k'):
    """用cv2依据帧间差选取关键帧\n
    ...
    idx_list = [i.idx for i in frames]
    diff_list = [i.diff for i in frames]
    # 得到目标帧的idx
    ...
    cap.release()
    return len(target_idx), target_idx, idx_list, diff_list
