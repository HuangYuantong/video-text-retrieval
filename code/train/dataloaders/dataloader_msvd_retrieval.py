...
class MSVD_DataLoader(Dataset):
    ...
    def _get_text(self, video_id, caption):
        ...
        pairs_text = numpy.zeros((k, self.max_words), dtype=int)
        pairs_mask = numpy.zeros((k, self.max_words), dtype=int)
        pairs_segment = numpy.zeros((k, self.max_words), dtype=int)
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            pairs_text[i] = numpy.array(input_ids)
            pairs_mask[i] = numpy.array(input_mask)
            pairs_segment[i] = numpy.array(segment_ids)

        return pairs_text, pairs_mask, pairs_segment, choice_video_ids

    def _get_rawvideo(self, choice_video_ids):
        video_mask = numpy.zeros((len(choice_video_ids), self.max_frames), dtype=int)
        max_video_length = [0] * len(choice_video_ids)

        # Pair x L x T x 3 x H x W
        video = numpy.zeros((len(choice_video_ids), self.max_frames, 1, 3,
                          self.rawVideoExtractor.size, self.rawVideoExtractor.size), dtype=numpy.float32)
            ...
            if len(raw_video_data.shape) > 3:
                ...
                if self.max_frames < raw_video_slice.shape[0]:
                    if self.slice_framepos == 0:
                        ...
                        sample_indx = numpy.linspace(0, raw_video_slice.shape[0] - 1, num=self.max_frames, dtype=int)
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(pairs_text,dtype=text_dtype), torch.tensor(video), torch.tensor(video_mask)
